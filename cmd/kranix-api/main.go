...
import (
	...
	"github.com/local/kronix-control-plane/internal/api"
	"github.com/local/kronix-control-plane/internal/core"
	kruntime "github.com/local/kronix-control-plane/internal/runtime"
)
...
func main() {
	...
	engine.Start(ctx)
	bus.Publish(core.Event{Level: "ok", Source: "api", Message: "Kranix API started"})
	...
	log.Printf("kranix-api listening on %s", *addr)
